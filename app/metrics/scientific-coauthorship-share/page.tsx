...
const DATA_PATH = '/data/scientific_coauthorship_share.json'
...
export default function ScientificCoauthorshipSharePage() {
    ...
    <main className="mx-auto max-w-4xl p-6 space-y-6">
      ...
      <header className="space-y-4">
        <div className="space-y-2">
          <p className="text-sm uppercase tracking-wide text-emerald-600">Truth &amp; Clarity · International</p>
          <h1 className="text-3xl font-semibold tracking-tight">Scientific co-authorship share (%)</h1>
          <p className="text-base text-slate-600">
            Share of articles with ≥2 affiliation countries among articles with ≥1 country (OpenAlex; journal/proceedings, published).
            Series spans {earliest && latest ? `${earliest.year}–${latest.year}` : '1990 onward'}.
          </p>
        </div>
        {latest ? (
          <div className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700">
            ...
            <span>·</span>
            <span>{latest.value.toFixed(1)}% of articles</span>
          </div>
        ) : null}
        <div className="flex flex-wrap gap-3 text-xs font-semibold uppercase tracking-wide">
          ...
          <span className="rounded-full bg-slate-100 px-3 py-1 text-slate-700">Unit: % of articles (1 decimal)</span>
        </div>
      </header>

      {status === 'loading' ? (
        <p className="rounded border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600">
          Loading scientific co-authorship share…
        </p>
      ) : null}
        ...
        <section className="card space-y-4 p-4 shadow-sm">
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={series}>
                ...
                <XAxis dataKey="year" />
                <YAxis tickFormatter={v => `${Number(v).toFixed(1)}%`} domain={['auto', 'auto']} />
          ...
          {latest ? (
            <footer className="space-y-2 text-sm text-slate-600">
              <p>
                Through {latest.year} (latest complete year). Source:{' '}
                <Link
                  href="https://api.openalex.org/works"
                  target="_blank"
...
function renderTooltipContent({ active, payload, label }: TooltipProps<number, string>) {
  ...
  return (
    <div className="rounded bg-white px-3 py-2 text-sm font-medium text-slate-900 shadow">
      {`${label} — ${value.toFixed(1)}% of articles`}
    </div>
  )
}
