...
export async function run() {
  ...
  for (const year of years) {
    ...
    let weighted = 0;
    for (const iso of isoWithBoth) {
      const pop = population.get(iso)!.get(year);
      const pov = poverty.get(iso)!.get(year);
      if (pop == null || pov == null) continue;
      if (Number.isNaN(pop) || Number.isNaN(pov)) continue;
      totalPop += pop;
      weighted += pop * pov;
    }
    if (totalPop > 0) {
      const mean = weighted / totalPop;
  ...
  console.log(
    `[extreme_poverty] raw rows: ${povertyRows.length} / population rows: ${populationRows.length} / ISO3 count: ${isoWithBoth.length} / years count: ${years.length} / computed points: ${data.length} range ${firstYear}-${lastYear}`,
  );
